...
def train(train_loader, plfd_backbone, auxiliarynet, criterion, optimizer,
    ...
    for iteration, (img, landmark_gt, euler_angle_gt) in tqdm(enumerate(train_loader), total=len(train_loader), desc="Training"):
        img = img.to(device)
...
def validate(wlfw_val_dataloader, plfd_backbone, auxiliarynet, criterion):
    ...
    nme = []
    with torch.no_grad():
        for img, landmark_gt, euler_angle_gt in tqdm(wlfw_val_dataloader, total=len(wlfw_val_dataloader), desc="Validation"):
            img = img.to(device)
            landmark_gt = landmark_gt.to(device)
            euler_angle_gt = euler_angle_gt.to(device)
            plfd_backbone = plfd_backbone.to(device)
